...
public class BestTime52 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (PlayerPrefs.HasKey("BestTime52"))
        {
            recordTime = PlayerPrefs.GetFloat("BestTime52");
        }
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.CompareTag("Player"))
        {
            if (timer != null && timer.timer < recordTime)
            {
                ...
                PlayerPrefs.SetFloat("BestTime52", recordTime);
                PlayerPrefs.Save();
                bestTimeText.text = "Best Time: " + recordTime.ToString("f2");
                if (recordTime < 61f)
                {
