...
public class BestTime61 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (PlayerPrefs.HasKey("BestTime61"))
        {
            recordTime = PlayerPrefs.GetFloat("BestTime61");
        }
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.CompareTag("Player"))
        {
            if (timer != null && timer.timer < recordTime)
            {
                ...
                PlayerPrefs.SetFloat("BestTime61", recordTime);
                PlayerPrefs.Save();
                bestTimeText.text = "Best Time: " + recordTime.ToString("f2");
                if (recordTime < 73f)
                {
