...
public class BestTime62 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (PlayerPrefs.HasKey("BestTime62"))
        {
            recordTime = PlayerPrefs.GetFloat("BestTime62");
        }
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.CompareTag("Player"))
        {
            if (timer != null && timer.timer < recordTime)
            {
                ...
                PlayerPrefs.SetFloat("BestTime62", recordTime);
                PlayerPrefs.Save();
                bestTimeText.text = "Best Time: " + recordTime.ToString("f2");
                if (recordTime < 72f)
                {
