...
public class BestTime83 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (PlayerPrefs.HasKey("BestTime83"))
        {
            recordTime = PlayerPrefs.GetFloat("BestTime83");
        }
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.CompareTag("Player"))
        {
            if (timer != null && timer.timer < recordTime)
            {
                ...
                PlayerPrefs.SetFloat("BestTime83", recordTime);
                PlayerPrefs.Save();
                bestTimeText.text = "Best Time: " + recordTime.ToString("f2");
                if (recordTime < 66f)
                {
