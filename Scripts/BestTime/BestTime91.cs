...
public class BestTime91 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (PlayerPrefs.HasKey("BestTime91"))
        {
            recordTime = PlayerPrefs.GetFloat("BestTime91");
        }
        ...
        if (collision.CompareTag("Player"))
        {
            if (candyCane.foundCandy)
            {
                if (timer != null && timer.timer < recordTime)
                {
                    ...
                    PlayerPrefs.SetFloat("BestTime91", recordTime);
                    PlayerPrefs.Save();
                    bestTimeText.text = "Best Time: " + recordTime.ToString("f2");
                    if (recordTime < 122f)
                    {
